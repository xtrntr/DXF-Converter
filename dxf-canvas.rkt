    ...
    (define/public (draw-objects lst) ;get a struct-list.
      (define (apply-procedure x)
        (when (entity-visible x)
          (match x
            [(line highlighted selected visible layer x1 y1 x2 y2)                           (draw-line x1 y1 x2 y2 (or selected highlighted))]
            [(arc highlighted selected visible layer x y radius start end x1 y1 x2 y2 x3 y3) (draw-arc x y radius start end (or selected highlighted))]
            [(point highlighted selected visible layer x y)                                  (draw-point x y (or selected highlighted))]
