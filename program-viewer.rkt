...
(define (open-file input-port a-frame)
  ...
  |#
    ...
    (new button%
         ...
         [parent button-panel-3]
         [callback (lambda (b e)
                     (set-field! search-list a-canvas (rotate-90 (get-field search-list a-canvas)))
                     (send a-canvas update-node-lst!)
                     (update-spreadsheet! (get-field search-list a-canvas))
                     (send a-canvas refocus!)
                     )])
    ...
    (new button%
         ...
         [parent button-panel-3]
         [callback (lambda (b e)
                     (set-field! search-list a-canvas (make-y-mirror (get-field search-list a-canvas)))
                     (send a-canvas update-node-lst!)
                     (update-spreadsheet! (get-field search-list a-canvas))
                     (send a-canvas refocus!)
                     )])
    ...
    (new button%
         ...
         [parent button-panel-3]
         [callback (lambda (b e)
                     (set-field! search-list a-canvas (make-x-mirror (get-field search-list a-canvas)))
                     (send a-canvas update-node-lst!)
                     (update-spreadsheet! (get-field search-list a-canvas))
                     (send a-canvas refocus!)
                     )])
