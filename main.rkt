...
(define (select-highlighted)
  (lambda (x) (when (entity-highlighted x)
  ...
  (class canvas%
    ...
    (define/override (on-char event)
      (let ((key (send event get-key-code)))
        ...
        (case key
          ['wheel-up    (set! global-x-scale (+ global-x-scale 0.2)) 
                        (set! global-y-scale (- global-y-scale 0.2))
          ...
          ['escape      (map (lambda (x) (set-entity-selected! x #f)) search-list)]
          ['wheel-down  (set! global-x-scale (- global-x-scale 0.2)) 
                        (set! global-y-scale (+ global-y-scale 0.2))
                        (send drawer set-transformation (vector transformation-matrix global-x-offset global-y-offset global-x-scale global-y-scale rotation))]
          ['#\backspace (map (lambda (x) (when (entity-selected x) (set-entity-visible! x #f) (set-entity-selected! x #f))) search-list)]))
      (send canvas refresh))
         ...
         (callback (lambda (checked e)
                     ...
                     (define (map-toggle-visibility struct-list)
                       (cond ((empty? struct-list) '())
                             ((path? (car struct-list)) (map-toggle-visibility (path-entities (car struct-list))))
